...
def fix_logical_formula_syntax(formula: str) -> str:
    """
    Fix common syntax errors in generated logical formulas.

    Args:
        formula: The generated logical formula

    Returns:
        Fixed logical formula
    """
    ...
    quantifier_pattern = r"(exists|forall)\s+([e\d\s\w]+)\."
    for match in re.finditer(quantifier_pattern, formula):
        var_list = match.group(2).strip()
        vars_in_quantifier = re.findall(r"[ex]\d+", var_list)
        declared_vars.update(vars_in_quantifier)

    var_refs = set(re.findall(r"[ex]\d+", formula))

    undefined_vars = var_refs - declared_vars
    if undefined_vars and declared_vars:
        defined_list = sorted(list(declared_vars))
        for undefined_var in undefined_vars:
            var_type = undefined_var[0]  # 'e' or 'x'
            candidates = [v for v in defined_list if v.startswith(var_type)]
            if candidates:
                replacement = candidates[0]
                formula = re.sub(
                    r"\b" + re.escape(undefined_var) + r"\b", replacement, formula
                )
...
def generate_predictions_for_csv(
    ...
    with tqdm(
        ...
        for i in range(0, len(texts), batch_size):
            ...
            fixed_predictions = [
                fix_logical_formula_syntax(pred) for pred in batch_predictions
            ]
            predictions.extend(fixed_predictions)
            pbar.update(1)
